...
export function JornaisGridSkeleton() {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4 lg:gap-6">
      {[0, 1, 2, 3].map((i) => (
        <div key={i} className="rounded-[16px] sm:rounded-[20px] border border-gray-100 overflow-hidden">
          <Skeleton className="h-36 sm:h-52 w-full bg-gray-200" />
          <div className="p-3 sm:p-4 space-y-2">
            <Skeleton className="h-3 w-20 bg-gray-100" />
            <Skeleton className="h-4 w-full bg-gray-200" />
            <Skeleton className="h-4 w-4/5 bg-gray-200" />
            <Skeleton className="h-8 sm:h-10 w-full rounded-lg bg-gray-100" />
          </div>
        </div>
      ))}
    </div>
